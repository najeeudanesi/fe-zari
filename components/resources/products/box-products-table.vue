    ...
    <div>
        ...
        <div class="w-full flex justify-end mb-4">
            <button class="bg-none outline font outline-1 outline-primary text-primary rounded-md px-4 py-2 flex"
                @click="exportCategoriesToXLSX(products?.data, 'Box Product Template')">
                <p>Export</p>
                <Icon :name="'export'" />
            </button>
        </div>
        ...
        <ResponsiveTable v-bind="{ data: products, skeleton: { rows: 3, columns: 7 } }" @paginate="paginate">
            <template #header>
                <th>
                    <ToggleSelections v-model="selected"
                        v-bind="{ data: products?.data, mapFn: (v: BoxProduct) => v.id, }" />
                </th>
                <th class="px-4 text-left">Image</th>
                <th class="px-4 text-left">Name</th>
                <th class="px-4 text-left">Max items</th>
                <th class="px-4 text-left">Min items</th>
                <th class="px-4 text-left">items</th>
                <th class="px-4 text-left">Actions</th>
            </template>
            <template #mobileHeader="{ row: product }: TableRow<BoxProduct>">
            ...
            <template #default="{ row: product }: TableRow<BoxProduct>">
                ...
                <TableCell2 class="flex items-center max-w-64" header="Image">
                    <div class="w-16 h-16  object-cover"><img
                ...
                <TableCell2 class="flex items-center" header="Max Items">
                    <div class="w-24">{{
                        product.maxAmount
                        }}</div>
                </TableCell2>
                <TableCell2 class="flex items-center" header="Min Items">
                    <div class="w-24">{{
                        product.minAmount
                        }}</div>
                </TableCell2>
        ...
        <SlideModal :open="modalOpen" staticBackDrop @close="toggleProductModal" title="Edit Product"
            ...
            <template #content>
                <AddBoxProduct @close="toggleProductModal" @submit="refresh" :categories="categories.data"
        ...
        <SlideModal :open="viewModal" staticBackDrop @close="toggleViewModal" :title="product.name"
            ...
            <template #content>
                <ViewBoxProduct @close="toggleViewModal" @submit="refresh" :categories="categories.data"
                    :product="product" :productUnits="unitOptions.data" :variantOptions="variantOptions.data" />
            </template>
        </SlideModal>
...
import { BoxProduct, BundledProduct, Product } from '~~/composables/resources/product'
...
import AddBoxProduct from './add-box-product.vue'
import ViewBoxProduct from './view-box-product.vue'
...
const {
    ...
    refresh,
    pending
} = useApiFetch<ApiList<BoxProduct>>('box-products', () => ({
    params: {
...
//     try {
//         const response = await apiFetch('/box-products', {
//             method: 'GET',
//         });
...
const showActionsMenu = (product: BoxProduct) => {
